<x-admin-layout>
    ...
    <div class="px-6 py-3 bg-white rounded-lg educations-container ">
        ...
        <form action="{{ route('company.update.categories', ['category' => $category]) }}" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="flex flex-col py-4 form">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="title" class="w-full bg-gray-100 border border-gray-200 rounded-md"
                        placeholder="Título" value="{{  $category->title ?? old('title') }}">
                </div>
