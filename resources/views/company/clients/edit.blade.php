<x-admin-layout>
    ...
    <div class="px-6 py-3 bg-white rounded-lg educations-container ">
        ...
        <form action="{{ route('company.update.clients', ['client' => $client]) }}" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="flex flex-col py-4 form">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="company" class="w-full bg-gray-100 border border-gray-200 rounded-md"
                        placeholder="Razão Social" value="{{ $client->company ?? old('company') }}">
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="fantasy" class="w-full bg-gray-100 border border-gray-200 rounded-md"
                        placeholder="Nome Fantasia" value="{{ $client->fantasy ?? old('fantasy') }}">
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="email" name="email" class="w-full bg-gray-100 border border-gray-200 rounded-md"
                        placeholder="E-mail" value="{{ $client->email ?? old('email') }}">
                </div>

                {{-- Document --}}
                <div class="mb-6">
                    <label for="document" class="block mb-1 font-semibold">CNPJ/CPF</label>
                    <input type="text" name="document" class="w-full bg-gray-100 border border-gray-200 rounded-md"
                        placeholder="CNPJ/CPF" value="{{ $client->document ?? old('document') }}">
                </div>
